...
mif2_run_seir <- function(seir_pomp, outbreak, graph_mif2 = TRUE) {
  ...
  dest <- paste0("/", outbrk, "_mif_alt.rda")
  ...
  dest <- paste0("/", outbrk, "_alt_mif_pf.rda")
...
prof_lik_run <- function(seir_pomp, outbrk, beta0) {
  ...
  seir_pomp
  b <- beta0
  prof_file <- paste0(outbrk, "_alt_prof_lik.rda")
  #b0 = 1.464, p=.0642080 
  ...
  if (file.exists(prof_file)) {
    ...
    sliceDesign(
      center=c(beta0 = b, sigma = 1/9.312799, 
               gamma = 1/7.411374, ff = 49/69, tau1 = 14.0,
               beta1 = 1, k = 1e-4),
      beta0=rep(seq(from=0.01,to=6,length=200),each=100)
    ) -> p
  ...
  #geomsmooth
  file_name <- paste0(outbrk,"_alt.pdf")
