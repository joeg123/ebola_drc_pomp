...
seir_combo_rprocess <- Csnippet('double N;
                          N = S+E+Il+Ih+R+D;
                          if(t == 71){ //Equator
                            S = 999997;
                            E = 0.0;
                            ...
                            D = 0.0;
                            C = 3.0;
                          } else if (t == 99 || t == 233 || t == 425 || t == 612 || t == 652){ //All others
                            S = 999998;
                            E = 0.0;
                            Il = 1.0;
                            Ih = 1.0;
...
generate_pomp_model <- function (data = NULL, sim=FALSE) {

  data %>% padr::pad() %>% 
    ungroup() %>% 
    mutate(day = seq_along(times)) %>% filter(!is.na(times)) %>% 
    select(times =day, cases) ->  data
  ...
  pomp(data = data,
       times="times",
       t0=-1,
       skeleton = vectorfield(seir_combo_skel), 
       params = seir_parm,
       initializer=init,
       ...
       rprocess = euler.sim(step.fun = seir_combo_rprocess, delta.t = 1)) -> ss_combo_seir_pomp
  
   
  return(ss_combo_seir_pomp)
}
